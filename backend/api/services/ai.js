...
export async function transcribeAudio(audioUrl) {
  ...
  return r.data.text || "Błąd transkrypcji";
}

export async function analyzeMeeting(t) {
  const r = await axios.post("https://api.groq.ai/openai/v1/chat/completions", {
    model: "llama-3.1-8b-instant",
    messages: [{role: "system", content: "Stwórz podsumowanie w stylu WhatsApp: Budżet, Termin, Status, Następny krok."}, {role: "user", content: t}],
    max_tokens: 200
  }, {headers: {Authorization: `Bearer ${GROQ_API_KEY}`}});
  return r.data.choices[0]?.message?.content || "Błąd analizy";
}
